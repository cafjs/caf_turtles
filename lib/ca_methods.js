...
exports.methods = {
    ...
    async __ca_pulse__() {
        try {
            ...
            if (this.state.counter % this.$.props.pulsesForFlex === 0) {
                await deployerUtil.autoFlex(this);
            }
    ...
    async addApp(appLocalName, image, isUntrusted, cdn) {
        ...
        try {
            const [err, [appInfo]] = await deployerUtil.getAppInfo(
                this, [appLocalName]
            );
            if (err) {
                err.checkApp = true;
                ...
                const id = this.$.deploy.createApp(
                    appLocalName, image, isUntrusted, plan, cdn || null, null
                );
                this.state.apps[appLocalName] = {
                    id, appLocalName, image, isUntrusted, cdn,
                    stat: null
                };
    ...
    async autoFlex() {
        assert(this.state.privileged, 'autoFlex() for privileged only');
        try {
            await deployerUtil.autoFlex(this);
            return this.getState();
        } catch(err) {
    ...
    async flexApp(appLocalName, numberOfCAs) {
        ...
        if (info) {
            assert(info.manual,
                   `Set manual mode for app flexing ${appLocalName}`);

            const [err, [appInfo]] = await deployerUtil.getAppInfo(
                this, [appLocalName]
            );
            if (err || !appInfo) {
                return [err || (new Error('Application not registered'))];
            } else {
                const {plan} = appInfo;
                const reqId = this.$.deploy.flexApp(info.id, plan, numberOfCAs);
                this.state.pending[reqId] = this.state.counter;
                return this.getState();
            }
        } else {
