...
exports.methods = {
    ...
    async addApp(appLocalName, image, isUntrusted) {
        ...
        try {
            ...
            const [err, appInfo] = await this.$.crossapp.dirtyCall(
                userApp, json_rpc.DEFAULT_FROM, 'getDeploymentInfo',
                [this.scratch.tokenStr, app]
            );
            if (err) {
                err.checkApp = true;
                return [err];
            } else {
                const id = this.$.deploy.createApp(appLocalName, image,
                                                   isUntrusted, appInfo.plan);
    ...
    async flexApp(appLocalName, instances) {
        // TO DO : THERE IS NO MORE FLEXING BASED ON #INSTANCES...
        if (!this.state.privileged) {
